...
def printCompleteTabela(tabela):
    print()
    print("{:19s} {:>3} {:>3} {:>3} {:>3} {:>3}:{:<3} {:>3}".format(
        "Equipa", "J", "V", "E", "D", "GM", "GS", "P"))
    for reg in tabela:
        nome, j, v, e, d, gm, gs, p = reg
        print("{:19s} {:3d} {:3d} {:3d} {:3d} {:3d}:{:<3d} {:3d}".format(
            nome, j, v, e, d, gm, gs, p))
...
###ATUALIZAR TABELA (COM NUMERO DE JOGOS E PONTOS)
complete_table = []
for reg in tabela:
    lst = list(reg)
    lst.insert(1, numJogos(reg))
    lst.append(pontos(reg))
    tup = tuple(lst)
    complete_table.append(tup)


##################################
# Mostra a tabela classificativa original, não ordenada:
print("Table não ordenada:")
printTabela(tabela)
print("--------------------------------------------------------------")
...
print("Table ordenada por pontos:")
tab = sorted(complete_table, key = lambda x: x[7], reverse=True)
printCompleteTabela(tab)
print("--------------------------------------------------------------")

# c)
# Acrescente os argumentos adequados à função sorted para
# obter uma tabela ordenada por ordem decrescente da diferença GM-GS:
print("Table ordenada por diferença de golos:")
tab = sorted(complete_table, key = lambda x: x[5]-x[6], reverse=True)
printCompleteTabela(tab)
print("--------------------------------------------------------------")
...
print("Table ordenada por pontos e/ou diferença de golos:")
tab = sorted(complete_table, key = lambda x: x[5]-x[6], reverse=True)
#printCompleteTabela(tab)
print("--------------------------------------------------------------")
